...
_monitor_push "{\"ts\":\"$_MONITOR_TS\",\"hook\":\"on-write\",\"type\":\"write\",\"project\":\"$_MONITOR_PROJ\",\"msg\":$(python3 -c "import sys,json;print(json.dumps(sys.argv[1]))" "$(basename $FILE) [write]" 2>/dev/null || echo "\"write\"")}"
